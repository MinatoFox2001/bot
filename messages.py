...
def get_subscription_info_text(user: dict) -> str:
    try:
        ...
        sub_limits = {
            'free': 20,          # 20 ПАР токенов
            'tier1': 20000,      # 20k ПАР токенов
            'tier2': 40000,      # 40k ПАР токенов
            'tier3': 100000      # 100k ПАР токенов
        }
        
        sub_name = sub_names.get(sub_type, 'Zenith Spark')
        limit = sub_limits.get(sub_type, 20)
        
        # Форматируем значения - ДЕЛИМ НА 2 потому что это ПАРЫ токенов
        ...
        # Формируем текст - отображаем ПАРЫ
        result = (
            f"📶 Подписка : {sub_name}\n\n"
            f"⚡️ Лимит : {format_tokens(limit//2)} пар токенов/день\n"  # Изменено: ПАРЫ вместо k
        )
