...
def get_welcome_message(user_id=None):
    ...
    if user_id is None:
        return (
            ...
            "⚠️ У вас подписка: Zenith Spark (20 токенов/день)\n"
    ...
    sub_limits = {
        'free': '20 токенов/день',
        'tier1': '20k токенов/день',
        'tier2': '40k токенов/день',
        'tier3': '100k токенов/день'
    }
    
    sub_name = sub_names.get(sub_type, 'Zenith Spark')
    sub_limit = sub_limits.get(sub_type, '20 токенов/день')
...
def get_profile_text(user: dict) -> str:
    try:
        ...
        # Формируем текст профиля с разделенными балансами
        result = (
            f"👤 Личный кабинет \n\n"
            f"🆔 {user_id}\n"
            f"📛Имя : {full_name}\n"
            f"🪪 Логин : @{username}\n"
            f"💰Баланс для покупок : {purchase_balance} руб.\n"
...
def get_subscription_info_text(user: dict) -> str:
    try:
        ...
        # Формируем текст - отображаем количество токенов
        result = (
            f"📶 Подписка : {sub_name}\n\n"
            f"⚡️ Лимит : {format_tokens(limit)} токенов/день\n"
        )
