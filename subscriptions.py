...
def get_subscriptions_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    if current_sub == 'tier3':
        ...
        buttons.append([InlineKeyboardButton(
            text="Zenith Eclipse - 700₽ (продление)", callback_data="sub_tier3")])
    elif current_sub == 'tier2':
        if active:
            # Nova активна - можно апгрейд до Eclipse
            buttons.append([InlineKeyboardButton(
                text="Zenith Eclipse - 700₽", callback_data="sub_tier3")])
        else:
            # Nova истекла - можно продлить или апгрейд
            buttons.append([InlineKeyboardButton(
                text="Zenith Nova - 500₽ (продление)", callback_data="sub_tier2")])
            buttons.append([InlineKeyboardButton(
                text="Zenith Eclipse - 700₽", callback_data="sub_tier3")])
        ...
        if active:
            ...
            buttons.append([InlineKeyboardButton(
                text="Zenith Nova - 500₽", callback_data="sub_tier2")])
            buttons.append([InlineKeyboardButton(
                text="Zenith Eclipse - 700₽", callback_data="sub_tier3")])
        else:
            # Pulse истекла - можно продлить или апгрейд
            buttons.append([InlineKeyboardButton(
                text="Zenith Pulse - 300₽ (продление)", callback_data="sub_tier1")])
            buttons.append([InlineKeyboardButton(
                text="Zenith Nova - 500₽", callback_data="sub_tier2")])
            buttons.append([InlineKeyboardButton(
                text="Zenith Eclipse - 700₽", callback_data="sub_tier3")])
        ...
        buttons.append([InlineKeyboardButton(
            text="Zenith Pulse - 300₽", callback_data="sub_tier1")])
        buttons.append([InlineKeyboardButton(
            text="Zenith Nova - 500₽", callback_data="sub_tier2")])
        buttons.append([InlineKeyboardButton(
            text="Zenith Eclipse - 700₽", callback_data="sub_tier3")])
...
async def show_subscriptions_menu(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            ...
            reply_markup=get_subscriptions_keyboard(callback.from_user.id)
        )
...
async def handle_subscription_selection(callback: CallbackQuery):
    ...
    try:
        ...
        # Показываем сообщение об успехе и автоматически возвращаем в профиль
        try:
            success_msg = await callback.message.answer(
                f"✅ Подписка {sub_name} успешно оформлена!\n\n"
                f"💰 Списано: {int(final_price)} руб. с баланса покупок"
                f"{f'\\n🎁 Применена скидка: {discount_percent}%' if discount_percent > 0 else ''}"
            )

            await callback.answer()

            # Удаляем сообщение об успехе через 2 секунды
            ...
            print(f"Ошибка при показе успеха: {e}")
            # В случае ошибки просто возвращаем в профиль
            from profile import show_clean_profile_menu
            await callback.answer()
            await show_clean_profile_menu(callback)
...
async def handle_back_to_profile(callback: CallbackQuery):
    ...
    try:
        ...
        from profile import get_profile_keyboard  # Локальный импорт
...
async def process_referral_bonuses(user_id: int, amount: float, sub_type: str):
    ...
    current_id = user_id
    print(
        f"Начало обработки реферальных бонусов для пользователя {user_id}, сумма: {amount}")
    ...
    for level in range(1, 4):  # Обрабатываем 3 уровня
        ...
        print(
            f"Начисление бонуса уровня {level}: {bonus} руб. рефереру {referrer_id}")
...
__all__ = ['show_subscriptions_menu', 'handle_subscription_selection',
           'handle_back_to_profile', 'get_subscriptions_keyboard']
